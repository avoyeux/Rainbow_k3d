...
class Plotting:
    ...
    def Plots(self, cubes_1, cubes_2, name):
        ...
        plt.scatter(self.dates, cubes_1, label=name[0], marker='x', color='blue')
        plt.scatter(self.dates, cubes_2, label=name[1], marker='x', color='orange')

        plt.title(f'Time evolution for {name[0]} and {name[1]}')
        plt.xlabel(f'Time in seconds')
        plt.ylabel(f'Voxel count')
        plt.legend()
        plt.savefig(os.path.join(self.paths['Saving'], f'Time_evo_{name[0][:-3]}.png'), dpi=100)

        # Normalised time evolution
        norm_1 = cubes_1 / cubes_1.max()
        norm_2 = cubes_2 / cubes_2.max()
        plt.figure(figsize=(16, 8))
        plt.scatter(self.dates, norm_1, label=name[0], marker='x', color='blue')
        plt.scatter(self.dates, norm_2, label=name[1], marker='x', color='orange')

        plt.title(f'Normalised time evolution for {name[0]} and {name[1]}')
        plt.xlabel(f'Time in seconds')
        plt.ylabel(f'Voxel count')
        plt.legend()
        plt.savefig(os.path.join(self.paths['Saving'], f'Norm_evo_{name[0][:-3]}.png'), dpi=100)
