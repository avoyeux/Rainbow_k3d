...
class Data:
    ...
    def Paths(self):
        ...
        self.paths = {'Main': main_path,
                      ...
                      'Intensities': os.path.join(main_path, 'data_dates', 'int'),
...
class K3dAnimation(Data):
    ...
    def __init__(self, compression_level=9, plot_height=1024, sleep_time=2, camera_fov=1, camera_zoom_speed=0.7, 
    ...
    def Update_paths(self):
        ...
        if self.make_screenshots:
            self.paths['Screenshots'] = os.path.join(self.paths['Main'], 'test_screenshot')
    ...
    def Screenshot_making(self):
        ...
        if self.time_intervals_all_data or self.time_intervals_no_duplicate:
            ...
            screenshot_name = f'Plot_{self.time_slider.value}_{self.version}.png'
    ...
    def Date_strings(self):
        ...
        self.date_text = [f'{date.year}-{date.month:02d}-{date.day:02d}  {date.hour:02d}h{date.minute:02d}min'
                          for date in self.dates]
    ...
    def Animation(self):
        ...
        if self.all_data:
            self.init_plot = k3d.voxels(self.cubes_all_data[0], outlines=False, 
                                        compression_level=self.compression_level, color_map=[0x8B0000], name='All data')
        ...
        if self.all_data:
            self.init_plot.voxels = {str(t): self.cubes_all_data[t] for t in range(self.cubes_all_data.shape[0])}
